...
#[tauri::command]
pub fn get_config_section(section: String, database: State<'_, Database>) -> Result<Value, String> {
...
#[tauri::command]
pub fn get_full_config(database: State<'_, Database>) -> Result<Value, String> {
    let conn = database.conn.lock().map_err(|e| e.to_string())?;
